                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('store') }}">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    <input id="creator" type="text" class="form-control @error('creator') is-invalid @enderror" name="creator" value="{{ old('creator') }}" autocomplete="creator" autofocus>
                                    @error('creator')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <option value="">Select Status</option>
                                        @foreach($statuses as $key => $status)
                                            <option value="{{ $key }}" {{ old('creator') == $key ? 'selected' : '' }}>{{ $status }}</option>
                                        @endforeach
                                    </select>
                                    @error('status')
                                        <span class="invalid-feedback" role="alert">
